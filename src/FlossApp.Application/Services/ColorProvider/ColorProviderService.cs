...
internal class ColorProviderService : IColorProviderService
{
    ...
    private async Task PopulateCacheForSchemaAsync(ColorSchema schema)
    {
        IColorFromJson[] fromFile = schema switch
        {
            ...
            ColorSchema.WplaceBasic => (await GetFromFileAsync<WplaceColor>("Wplace.json")).Where(x => !x.IsPremium).Cast<IColorFromJson>().ToArray(),
            ColorSchema.WplaceFull => (await GetFromFileAsync<WplaceColor>("Wplace.json")).Cast<IColorFromJson>().ToArray(),
